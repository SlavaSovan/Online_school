...
from apps.lessons.filters import (
    ...
    AdminLessonFilter,
    AdminLessonTaskFilter,
)

from .models import Lesson, LessonContent, LessonTask
from .serializers import LessonContentSerializer, LessonSerializer, LessonTaskSerializer
...
class LessonTaskListView(generics.ListCreateAPIView):
    """Список задач урока + создание новой"""
    ...
    def get_queryset(self):
        ...
        return LessonTask.objects.filter(lesson=lesson).order_by("order")
    ...
    def check_lesson_access(self, lesson):
        ...
        if lesson.is_published and course.status == "published":
            if (
                user_id
                and EnrollmentCache.objects.filter(
                    user_id=user_id, course=course.id
                ).exists()
            ):
                return True
        ...
        raise PermissionDenied("You don't have access to this lesson tasks")
...
class LessonTaskDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        ...
        return LessonTask.objects.filter(lesson=lesson)
    ...
    def check_lesson_access(self, lesson):
        ...
        raise PermissionDenied("You don't have access to this lesson task")
...
class AdminLessonTaskListView(generics.ListCreateAPIView):
    """Получение списка всех задач для администратора с фильтрацией"""
    ...
    filterset_class = AdminLessonTaskFilter
    search_fields = ["title", "external_task_uuid"]
    ...
    def get_queryset(self):
        """Получаем все задачи с оптимизацией"""
        return LessonTask.objects.select_related(
            "lesson", "lesson__module", "lesson__module__course"
        ).all()
    ...
    def perform_create(self, serializer):
        ...
        external_task_uuid = serializer.validated_data.get("external_task_uuid")

        if LessonTask.objects.filter(
            lesson=lesson, external_task_uuid=external_task_uuid
        ).exists():
            raise ValidationError(
                {
                    "external_task_uuid": f"Task with UUID '{external_task_uuid}' already exists in this lesson"
                }
            )
...
class AdminLessonTaskDetailView(generics.RetrieveUpdateDestroyAPIView):
    """Детальное управление задачей для администратора"""

    queryset = LessonTask.objects.select_related("lesson").all()
    serializer_class = LessonTaskSerializer
    permission_classes = [IsAdmin]
    lookup_field = "id"
    ...
    def perform_update(self, serializer):
        ...
        if "external_task_uuid" in serializer.validated_data:
            new_uuid = serializer.validated_data["external_task_uuid"]
            lesson = serializer.validated_data.get("lesson", instance.lesson)

            if (
                LessonTask.objects.filter(lesson=lesson, external_task_uuid=new_uuid)
                .exclude(pk=instance.pk)
                .exists()
            ):
                raise ValidationError(
                    {
                        "external_task_uuid": f"Task with UUID '{new_uuid}' already exists in this lesson"
                    }
                )
