...
class AdminLessonTaskFilter(django_filters.FilterSet):
    """Фильтры для задач уроков в админ-панели"""
    ...
    external_task_uuid = django_filters.CharFilter(
        field_name="external_task_uuid", lookup_expr="icontains"
    )
    ...
    def filter_search(self, queryset, name, value):
        """Поиск по названию задачи, урока или курса"""
        return queryset.filter(
            Q(title__icontains=value)
            | Q(lesson__title__icontains=value)
            | Q(lesson__module__course__title__icontains=value)
        )
